...
local color = ERM_UnitHelper.format_team_color(
        settings.startup[MOD_NAME..'-team_color'].value,
        settings.startup[MOD_NAME..'-team_blend_mode'].value
)

local disable_mask = false

local preserve_gloss = false
if settings.startup[MOD_NAME..'-team_color_preserve_gloss'].value then
    preserve_gloss = true
